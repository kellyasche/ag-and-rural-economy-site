...
Overall, decreases in farm employment have been a trend dating back to 2001. There is evidence that the crop commodity boom slowed the decline between 2008 and 2012, however the declines have continued since that period. 

Overall, farm employment makes up a small percentage of total employment across most of the state. However, in our most rural counties farm employment averages 15% of total employment. 

In addition, identifying any declines in total employment related to declines in farm employment are difficult to identify due to an overall shrinking workforce in rural areas. However, an examination of the data provided the following guidance on identifying which counties have been possibly impacted by declines in farm employment;

* they are likely located in the Northwest, Central, or Southwest planning regions;
* have 10% or more of their employment in farming; and
* have had their employment peak before 2016.

There were 23 counties that fell in these thresholds. After examining the trends in these counties, 14 counties were identified as possibly being impacted by the decline in farm employment.
...
```{r table of most disrupted counties summary}
table.disrupted.counties.summary <- read_csv("Data/Employment/farm-emp-table-for-site-summary.csv") %>%
  filter(Name %in% c("Norman", "Marshall", "Mahnomen", "Wilkin", "Grant", "Renville", "Swift", "Yellow Medicine", "Big Stone", "Lac qui Parle", "Lincoln", "Cottonwood", "Redwood", "Faribault"))

datatable(table.disrupted.counties.summary, class = "cell-border stripe", filter = "top", rownames = FALSE,
...
```{r prep most disrupted counties map}
map.disrupted.counties <- table.disrupted.counties.summary %>%
  left_join(counties.regions[,c(1,2)], by = c("Name")) %>%
  right_join(mn_counties[,c(4,7)], by = c("countyfp" = "FIPS_CODE")) %>%
  mutate(impacted.cat = ifelse(is.na(`Change in farm employment since 2001`), "Not likely impacted by farm employment", "Likely impacted by farm employment"))

```

```{r map impacted counties}

map.disrupted.counties.plot <- ggplot(map.disrupted.counties) +
  geom_sf_interactive(aes(fill = impacted.cat, tooltip = paste(Name, "\nFarm employment as % of total employment in 2018: ", percent(`Farm employment as % of total`), "\nChange in farm employment since 2001: ", percent(`Change in farm employment since 2001`), "\nChange in total employment since 2001: ", percent(`Change in total employment since 2001`), sep = ""), data_id = Name, geometry = geometry)) +
  scale_fill_manual(values = c("#5CA81F", "white"))+
  theme_sf+
  labs(title="Counties most likely impacted\nby farm employment") +
  theme(text = element_text(size = 14))

girafe(ggobj = map.disrupted.counties.plot, width_svg = 8) %>%
...
**Definition of Farm employment:** the number of workers engaged in the direct production of agricultural commodities, either livestock or crops; whether as a sole proprietor, partner, or hired laborer (Bureau of Economic Analysis).

<br>

# Analysis

This analysis examines the impact farm employment has on total employment across Minnesota. We do this through the following sections;

**Change in farm employment vs. total employment since 2001**: We will examine the trends in farm employment and identify any regions whose total employment shows similar trends. This would mean that farm employment has an out sized impact on the total employment for the region.

**Farm employment as a % of total employment**: If there are trends in farm employment that are concerning, it is likely going to have the most impact in counties with a higher percentage of their total employment in farming. We will seek to identify those regions.

**Counties - Peak year of total employment vs. percentage of employment in farming**: Attempting to find relationships and conclusions about employment in rural Minnesota is tricky because demographic shifts resulting in retirements can look similar to declines in farm employment. We will examine counties that are currently experiencing declines in their total employment and see if these counties also have a higher percentage of their employment in farming.

**Counties most disrupted by farm employment**: Through the above analysis, we should begin to get the main characteristics of a county where farm employment is having a larger impact on total employment - the most likely planning region and EDRs the counties are located in, as well as some measurements and thresholds of their farm and total employment. This section will seek to identify those exact counties.
...
Farm employment has been decreasing all across the state, and at similar rates. The crop commodity boom staved off the decreases, indicated by the plateaus between 2008 and 2012, but has since decreased through 2018. In 2018 farm employment levels were between 70% and 76% of their 2001 levels.

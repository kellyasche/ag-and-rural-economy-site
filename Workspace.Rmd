...
## Counties most disrupted by declines in farm employment
...
```{r farm and total employment index in most disrupted counties, include=FALSE, cache=TRUE}
...
farm.total.emp.disrupted.counties <- master.farm.emp.county %>%
  right_join(disrupted.counties, by = "countyfp") %>%
  group_by(countyfp, Name) %>%
  mutate(farm.emp.index = farm.emp / farm.emp[year == 2001],
         total.emp.index = total.emp / total.emp[year == 2001]) %>%
  ungroup()
...
```{r charts farm and total employment index in most disrupted counties, fig.height=10}

farm.emp.disrupted.counties.plot <- ggplot(farm.total.emp.disrupted.counties, aes(year, farm.emp.index, color = Name)) +

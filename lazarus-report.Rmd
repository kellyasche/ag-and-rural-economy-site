...
* are located in Southwest Minnesota and have 3% or more of the employment in industries identified in the Lazarus studet at some point since 2001.
...
On the otherhand, Marshall and Wlkin Counties have experienced some healthy growth in these industries over the last few years. 
...
What are the industries that are driving the increases and decreases in these counties?
...
mean.change.impact.counties <- impacted.counties %>%
  filter(periodyear > 2009) %>%
  group_by(naicstitle, areaname, countyfp) %>%
  arrange(periodyear) %>%
  mutate(pct.change = (empYear - lag(empYear))/lag(empYear)) %>%
  ungroup() %>%
  group_by(naicstitle, areaname, countyfp) %>%
  summarise(mean.change = mean(pct.change, na.rm = TRUE)) %>%
  ungroup() %>%
  drop_na %>%
  arrange(areaname)
```

### Counties with increasing employment in Lazarus industries

Analysis here

<br>

```{r charts lazarus industry trends increasing counties}
